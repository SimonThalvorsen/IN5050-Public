...
#include <immintrin.h> // AVX -- should again include #include <avxintrin.h>
...
// We already assume the use of a cosine table, normalization table and SSE for the DCT
...
static float dct_norm_table[8][8]; // This table is symetrical
...
static float costable[8][8]; // This table is NOT symetrical
        ...
        uint32_t padheight, uint8_t *quantization)
{
    ...
    for(y = 0; y < height; y += 8)
    {
        ...
        for(x = 0; x < width; x += 8)
        {
	        ...
	        for(v = 0; v < 8; ++v)
                {
                    /* Compute the DCT */
		    float dct = 0;
		    __m256 dct_vec = _mm256_setzero_ps(); 

                    for(j = 0; j < jj; ++j) 
		    {  
		      __m256 coeff, cos_jv, cos_iu, const128;
		      //		      __m256 coeff_v1, coeff_v2, cos_jv, cos_iu1, cos_iu2, const128;

		      const128 = _mm256_set1_ps(128.0f);   // the 128.0f constant
		      int tmp = ((y+j)*width)+x;        // the static offset in the loop, only i varies

		      // We operate on 32-bit floats --> the 256 bit can hold 8

		      coeff = _mm256_set_ps(in_data[tmp+0], // read in 8 elements from in_data
					    in_data[tmp+1],
					    in_data[tmp+2],
					    in_data[tmp+3],
					    in_data[tmp+4],
		      ...
		      coeff = _mm256_sub_ps(coeff, const128);

		      // costable[j][v] - static for the inner loop
		      cos_jv = _mm256_set1_ps(costable[j][v]);

		      // costable[i][u] - i varies from 0-7

		      cos_iu = _mm256_set_ps(costable[0][u], 
					   costable[1][u], 
					   costable[2][u], 
					   costable[3][u], 
					   costable[4][u], 
		      ...
		      // If costable was transposed, we could use
      		      //_mm256_store_ps(cos_iu, costable[u][0]); 
		      // but seems to be equally fast

		      // Multiply the cosine values
		      cos_iu = _mm256_mul_ps(cos_iu, cos_jv);

		      // Multiply with the coeff values
		      coeff = _mm256_mul_ps(cos_iu, coeff);

		      // Add the 8 coeffs to the dct_vec - adding the 8x8 matrix row by row first
		      dct_vec = _mm256_add_ps(coeff, dct_vec);
		    }
		    ...
		    dct_vec = _mm256_hadd_ps (dct_vec, dct_vec);
		    dct_vec = _mm256_hadd_ps (dct_vec, dct_vec);  // All elements now contain the sum of the coeffs
		    dct_vec = _mm256_add_ps(dct_vec, _mm256_permute2f128_ps(dct_vec , dct_vec , 1));  
		    
		    _mm256_store_si256((void*)dct_tmp, (__m256i)dct_vec);
		    dct += dct_tmp[0]; // or any of the 8
		    

		    // Scale according to normalizing function and quantize
