    ...
    for(y = 0; y < height; y += 8)
    {
        ...
        for(x = 0; x < width; x += 8)
        {
		    ...
		    {  
		      ...
		      // Add the 8 coeffs to the dct
#if 0
		      // "manually" add all 8
		      float dct_tmp[8];

		      _mm256_store_si256((void*)dct_tmp, (__m256i)coeff);
		      dct += dct_tmp[0] + dct_tmp[1] + dct_tmp[2] + dct_tmp[3] + 
			     dct_tmp[4] + dct_tmp[5] + dct_tmp[6] + dct_tmp[7];		      
#else
		      // add using hadd
		      float dct_tmp[8];
		      // A triple hadd does not scale (takes the upper and lower 128-bit hadd separately) 
		      // Use the permute

		      coeff = _mm256_hadd_ps (coeff, coeff);
		      coeff = _mm256_hadd_ps (coeff, coeff);  // All elements now contain the sum of the coeffs
      		      coeff = _mm256_add_ps(coeff, _mm256_permute2f128_ps(coeff , coeff , 1));  
