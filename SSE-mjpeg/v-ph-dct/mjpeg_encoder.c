...
#define COSINE_TABLE   1  // use a cosine table instead of calculating the values each time
#define DCT_SSE        1  // use sse in inner loop of dct, depends on the COSINE_TABLE
#define DCT_NORM_TABLE 1  // use a normalizing table instead of calculating the values each time  
...
#if  DCT_SSE==1 && COSINE_TABLE==1
                    for(j = 0; j < jj; ++j) 
		    {  
		      __m128 coeff_v1, coeff_v2, cos_jv, cos_iu1, cos_iu2, const128;

		      // ** REPLACE FOOR-LOOP OVER: float coeff = in_data[(y+j)*width+(x+i)] - 128.0f;

		      const128 = _mm_set_ps1(128.0f);   // the 128.0f constant
		      int tmp = ((y+j)*width)+x;        // the static offset in the j loop, only i varies

		      // We operate on 32-bit floats --> the 128 bit can hold 4
		      coeff_v1 = _mm_set_ps(in_data[tmp+0], // read in first 4 (0-3) elements from in_data
					     in_data[tmp+1],
					     in_data[tmp+2],
					     in_data[tmp+3]);

		      coeff_v2 = _mm_set_ps(in_data[tmp+4], // read in next 4 (4-7) elements from in_data
					     in_data[tmp+5],
					     in_data[tmp+6],
					     in_data[tmp+7]);
		      ...
		      // ** REPLACEE FOOR-LOOP OVER: dct += coeff * (float) costable[i][u] * costable[j][v];
		      ...
		      // Add the Add
		      // coeffscal pairs of coeff-values
		      coeff_v1 = _mm_add_ps(coeff_v1, coeff_v2);

		      // Make a horisontal add of the last 4 elements
		      // Could also be done using multiple (3) HADDPS - _mm_hadd_ps(a,b)
		      // a(A3,A2,A1,A0) + b(B3,B2,B1,B0) = (B3+B2, B1+B0, A3+A2, A1+A0).
		      //
		      // --> coeff_v1 = _mm_hadd_ps(coeff_v1, coeff_v1)
		      // --> coeff_v1 = _mm_hadd_ps(coeff_v1, coeff_v1)
		      // ==> coeff_v1 contains the same sum-of-dct in all 4 32-bit elements 

		      float dct_tmp[4];
		      _mm_store_si128((void*)dct_tmp, (__m128i)coeff_v1);
...
#if COSINE_TABLE==1
                            dct += coeff * (float) costable[i][u] * costable[j][v];
#else
                            dct += coeff * (float) (cos((2*i+1)*u*PI/16.0f) * cos((2*j+1)*v*PI/16.0f));
#endif // COSINE_TABLE
                        }
#endif // DCT_SSE && COSINE_TABL

		    
		    // The normalizing and the quantize can also be moved outside the next loop
		    // --> collect 8 DCT-values into 2 _m128-variables - the v-loop
		    // --> run the normalizing and the quantize over these two vectors
		    // --> next time ....
		    // --> ... or it ca be perfomed every 4th calculation of dct ("if (mod(u+1,4)==0))
#if DCT_NORM_TABLE == 1
		    // OLD code without a table
		    //float a1 = !u ? ISQRT2 : 1.0f;
                    //float a2 = !v ? ISQRT2 : 1.0f;
		    //dct *= a1*a2/4.0f;    // Scale according to normalizing function 
		    // use the lookup table instead of calculating the values
		    dct *= dct_norm_table[u][v]; // Scale according to normalizing function 
#else
		    // calculate the normalizing values and normalize the dct
                    float a1 = !u ? ISQRT2 : 1.0f;
                    float a2 = !v ? ISQRT2 : 1.0f;
		    dct *= a1*a2/4.0f;    // Scale according to normalizing function 
#endif //DCT_NORM_TABLE
